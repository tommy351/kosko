...
When you run [`kosko generate`](../cli/generate.mdx), Kosko will find all files or folders in `components` folder that matching the pattern set in [`components`](../configuration.mdx#components) configuration (it is `*` by default, which means everything in components folder). You can add component names or patterns to the command to specify which components to generate.
...
Then you can run [`kosko generate`](../cli/generate.mdx) to print YAML.
...
At the last line, we replace the export value with an array. So when you run [`kosko generate`](../cli/generate.mdx), it will print YAML as below.
...
<AvailabilityInfo availability={{ kosko: "1.0.0" }} />
